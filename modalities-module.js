(() => {
  ...
  function addStyles(){if(document.getElementById('stackup-modalities-style'))return;const st=document.createElement('style');st.id='stackup-modalities-style';st.textContent=`
    ...
    .m2-training-shell{margin-top:20px}.m2-training{margin-top:0;border:2px solid var(--gold,#d4aa58);border-radius:20px;overflow:hidden;background:var(--b2,#2a160d);box-shadow:0 12px 28px #0003;color:var(--w,#f8f0df)}.m2-head{padding:17px 16px 14px;background:linear-gradient(180deg,#2f1a10,#211008);border-bottom:1px solid #d4aa5850}.m2-kicker{display:block;color:var(--gold,#d4aa58);font-size:var(--type-meta,12px);letter-spacing:.09em;text-transform:uppercase;margin-bottom:3px}.m2-head h3{margin:0;color:var(--w,#f8f0df);font-size:var(--type-section,24px);line-height:1.08;text-transform:uppercase}.m2-head p{margin:7px 0 0;color:#cfbda7;font-size:var(--type-small,14px);line-height:1.45}.m2-modes{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}.m2-mode{padding:5px 8px;border:1px solid #d4aa5866;border-radius:9px;color:#d8c6ad;font-size:var(--type-micro,11px);background:#211008}
    .m2-stats{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;width:100%;max-width:100%;min-width:0;box-sizing:border-box;margin:0 0 12px;padding:0;overflow:visible;border:0;border-radius:0;background:transparent}.m2-stat{min-width:0;min-height:68px;padding:9px 4px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;box-sizing:border-box;overflow:hidden;border:1px solid #d4aa5870;border-radius:14px;background:linear-gradient(180deg,#2f1a10,#211008)}.m2-stat+.m2-stat{border-left:1px solid #d4aa5870}.m2-stat:last-child{background:linear-gradient(180deg,#195f4c,#08372d);border-color:#d4aa58aa}.m2-stat span{display:block;width:100%;margin:0 0 5px;color:#d8c6ad;font-size:clamp(8px,2.35vw,10px);line-height:1.05;letter-spacing:.03em;text-transform:uppercase;white-space:normal;overflow-wrap:anywhere}.m2-stat strong{display:block;width:100%;max-width:100%;font-size:clamp(12px,3.55vw,17px);line-height:1.05;letter-spacing:-.035em;color:var(--gold,#d4aa58);font-variant-numeric:tabular-nums;white-space:normal;overflow-wrap:anywhere}@media(max-width:390px){.m2-stats{gap:6px;margin-bottom:10px}.m2-stat{min-height:64px;padding:8px 2px}.m2-stat span{font-size:clamp(8px,2.25vw,9px);letter-spacing:.015em}.m2-stat strong{font-size:clamp(12px,3.45vw,14px);letter-spacing:-.045em}}
    .m2-body{padding:14px}.m2-spot{background:var(--c,#f2ead8);border:1px solid var(--gold2,#a87c32);border-radius:17px;color:var(--ink,#25170f);overflow:hidden}.m2-spotbar{display:flex;justify-content:space-between;gap:8px;padding:9px 11px;background:#e7dcc2;border-bottom:1px solid #a87c3244}.m2-spotbar span{font-size:var(--type-meta,12px);color:#725f4d}.m2-type{padding:5px 7px;border-radius:8px;background:#211008!important;color:#d4aa58!important}.m2-question{padding:15px 14px 10px;font-size:var(--type-question,18px);line-height:1.42}.m2-options{display:grid;gap:8px;padding:4px 12px 14px}.m2-option{width:100%;min-height:46px;border:1px solid #a87c3266;border-radius:12px;background:#f8f0df;color:#25170f;text-align:left;padding:10px 12px;font:inherit;font-size:var(--type-body,16px);cursor:pointer}.m2-option.picked{border:2px solid #725f4d;background:#eadfc8}.m2-option.correct,.m2-option.picked.correct{border:2px solid #0e4b3b!important;background:#dce9df!important;color:#08372d!important}.m2-option.wrong,.m2-option.picked.wrong{border:2px solid #9f3028!important;background:#f2d6d2!important;color:#6b1712!important}.m2-seqnum{display:inline-grid;place-items:center;width:25px;height:25px;border-radius:8px;background:#211008;color:#d4aa58;margin-right:8px;font-size:var(--type-meta,12px)}
  ...
  function sequenceComment(s,a){if(s.type!=='sequence'||!Array.isArray(a.selected)||!Array.isArray(s.answer))return '';if(a.correct)return 'Sequência correta. A ordem foi reconstruída sem inversões.';const i=s.answer.findIndex((v,x)=>a.selected[x]!==v);return i>=0?`A sequência divergiu na posição ${i+1}: você marcou ${a.selected[i]??'—'}, mas o correto é ${s.answer[i]}.`:'Revise a ordem dos itens e tente novamente.';}
  function feedback(s,a){const sel=Array.isArray(a.selected)?a.selected.join(' → '):a.selected,ans=Array.isArray(s.answer)?s.answer.join(' → '):s.answer,comment=sequenceComment(s,a);return `<div class="m2-feedback"><div class="m2-result ${a.correct?'ok':'no'}">${a.correct?'✓ RESULTADO: ACERTO':'✕ RESULTADO: REVISAR'}</div><div class="m2-analysis"><strong>SUA RESPOSTA:</strong> ${esc(sel)}<br><strong>RESPOSTA CORRETA:</strong> ${esc(ans)}<br><br><strong>ANÁLISE:</strong> ${esc(s.analysis)}${comment?`<div class="m2-comment"><strong>COMENTÁRIO:</strong> ${esc(comment)}</div>`:''}</div></div>`;}
  function renderTraining(shell,n){const rt=runtime(n);if(rt.cursor<0)next(rt,n);const s=current(rt,n),p=progress(n).answers||{},saved=p[s.id],locked=!!saved&&!rt.redo,st=stats(n);let opts='';if(s.type==='choice'||s.type==='binary'){opts=seeded(s.options,s.id).map(o=>{let c='m2-option';if(locked){if(o===s.answer)c+=' correct';if(saved.selected===o&&o!==s.answer)c+=' wrong';if(saved.selected===o)c+=' picked';}return `<button class="${c}" data-m2-answer="${esc(o)}" ${locked?'disabled':''}>${esc(o)}</button>`;}).join('');}else{const items=seeded(s.items,s.id),sel=locked?(Array.isArray(saved.selected)?saved.selected:[]):rt.seq;opts=items.map(o=>{const x=sel.indexOf(o),expected=s.answer.indexOf(o);let cls='m2-option';if(x>=0)cls+=' picked';if(locked&&x>=0)cls+=x===expected?' correct':' wrong';return `<button class="${cls}" data-m2-seq="${esc(o)}" ${locked?'disabled':''}>${x>=0?`<span class="m2-seqnum">${x+1}</span>`:''}${esc(o)}</button>`;}).join('');}
    const pos=BANK[n].findIndex(x=>x.id===s.id)+1;shell.innerHTML=`<div class="m2-stats"><div class="m2-stat"><span>CERTOS</span><strong>${st.c} · ${st.cp}%</strong></div><div class="m2-stat"><span>REALIZADOS</span><strong>${st.r} · ${st.rp}%</strong></div><div class="m2-stat"><span>TOTAL</span><strong>50 · 100%</strong></div></div><div class="m2-training"><div class="m2-head"><span class="m2-kicker">PRÁTICA INTERATIVA · 50 SPOTS</span><h3>TREINE ESTA MODALIDADE</h3><p>Reconheça regras, estrutura, formação de mãos, ordem de ação e procedimentos. Os spots ainda não realizados têm prioridade.</p><div class="m2-modes"><span class="m2-mode">ESCOLHA</span><span class="m2-mode">CERTO / ERRADO</span><span class="m2-mode">ORDEM</span></div></div><div class="m2-body"><div class="m2-spot"><div class="m2-spotbar"><span>SPOT ${String(pos).padStart(2,'0')} / 50</span><span class="m2-type">${s.type==='choice'?'ESCOLHA':s.type==='binary'?'CERTO / ERRADO':'COLOQUE EM ORDEM'}</span></div>${visual(n,s.visual)}<div class="m2-question">${esc(s.prompt)}</div><div class="m2-options">${opts}</div>${locked?feedback(s,saved):''}</div><div class="m2-nav"><button class="m2-btn" data-m2-prev ${rt.cursor<=0?'disabled':''}>‹ ANTERIOR</button><button class="m2-btn" data-m2-redo ${!saved?'disabled':''}>↻ REFAZER</button><button class="m2-btn primary" data-m2-next>PRÓXIMO ›</button></div>${st.r===50?'<div class="m2-complete">MODALIDADE TREINADA: 50/50 SPOTS REALIZADOS. Continue em PRÓXIMO para revisar em nova ordem.</div>':''}</div></div>`;bind(shell,n);}
