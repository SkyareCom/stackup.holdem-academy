(() => {
  ...
  function renderShell(shell,chapter){
    ...
    if(spot.type==='choice'||spot.type==='binary'){
      ...
      optionHtml=items.map(o=>{const idx=selected.indexOf(o),expected=spot.answer.indexOf(o);let cls='fi-option';if(idx>=0)cls+=' fi-picked';if(locked&&idx>=0)cls+=idx===expected?' fi-correct':' fi-wrong';return `<button class="${cls}" data-seq="${esc(o)}" ${locked?'disabled':''}>${idx>=0?`<span class="fi-seqnum">${idx+1}</span>`:''}${esc(o)}</button>`;}).join('');
    }
  ...
  function feedbackComment(spot,saved){if(spot.type==='sequence'&&Array.isArray(saved.selected)&&Array.isArray(spot.answer)){if(saved.correct)return 'Sequência correta. A ordem foi reconstruída sem inversões.';const i=spot.answer.findIndex((v,x)=>saved.selected[x]!==v);if(i>=0)return `A sequência divergiu na posição ${i+1}: você marcou ${saved.selected[i]??'—'}, mas o correto é ${spot.answer[i]}.`;return 'Revise a ordem dos itens e tente novamente.';}return saved.correct?'Boa decisão. Você aplicou corretamente o conceito deste spot. Use PRÓXIMO para receber outra situação sorteada.':'Este ponto ainda merece revisão. Leia a análise, use REFAZER e tente aplicar a regra sem depender da resposta anterior.';}
  function feedbackHtml(spot,saved){const selected=Array.isArray(saved.selected)?saved.selected.join(' → '):saved.selected;const answer=Array.isArray(spot.answer)?spot.answer.join(' → '):spot.answer;return `<div class="fi-feedback"><div class="fi-result ${saved.correct?'ok':'no'}">${saved.correct?'✓ RESULTADO: ACERTO':'✕ RESULTADO: REVISAR'}</div><div class="fi-analysis"><strong>SUA RESPOSTA:</strong> ${esc(selected)}<br><strong>RESPOSTA CORRETA:</strong> ${esc(answer)}<br><br><strong>ANÁLISE:</strong> ${esc(spot.analysis)}<div class="fi-comment"><strong>COMENTÁRIO:</strong> ${esc(feedbackComment(spot,saved))}</div></div></div>`;}
  function grade(shell,chapter,spot,selected){const correct=sameAnswer(selected,spot.answer);const previous=chapterProgress(chapter).answers?.[spot.id];saveAnswer(chapter,spot,selected,correct);if(!previous&&window.StackupAcademyProgression)window.StackupAcademyProgression.award({id:'fundamentals:'+chapter+':'+spot.id,correct,difficulty:spot.difficulty||'basic',stage:'fundamentals'});const rt=runtime(chapter);rt.redo=false;rt.seq=[];renderShell(shell,chapter);}
