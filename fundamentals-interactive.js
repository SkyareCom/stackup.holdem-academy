(() => {
  ...
  function statData(chapter){const answers=chapterProgress(chapter).answers||{};const vals=Object.values(answers);const realized=vals.length;const correct=vals.filter(x=>x.correct).length;return {correct,realized,correctPct:realized?Math.round(correct/realized*100):0,realizedPct:Math.round(realized/50*100)};}
  ...
  function renderShell(shell,chapter){
    ...
    const stats=statData(chapter);const progress=chapterProgress(chapter).answers||{};const saved=progress[spot.id];const locked=!!saved&&!rt.redo;const pos=(BANK()[chapter]||[]).findIndex(s=>s.id===spot.id)+1;let optionHtml='';
