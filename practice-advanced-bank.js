(() => {
  ...
  // Cada conceito aparece uma única vez. Variar apenas o prefixo de uma pergunta
  // não cria uma atividade inédita e causava repetição perceptível no Quiz.
  const QUIZ=FACTS.map((f,fi)=>({
    id:`Q${String(fi+1).padStart(3,'0')}`,
    question:f[0],
    options:[f[1],...f[2]],
    answer:f[1],
    why:f[3],
